 ...
 * USA 
 *
 * $Id: term.c,v 1.1 2007/03/03 07:47:00 dwight Exp $
 */
...
term_find (int fd)
{
	...
	lgdprintf(D_TERM, "term_find: returns %d for fd=%d\n", rval, fd);
	return rval;
}
...
term_exitfunc (void)
{
	...
	lgdprintf(D_TERM, "term_exitfunc: called\n");
	do { /* dummy */
		if (term.init != 1)
			break;

	lgdprintf(D_TERM, "term_exitfunc: closing fds:\n");
		for (i = 0; i < MAX_TERMS; i++) {
			if (term.fd[i] <= 0)
				continue;
			do { /* dummy */
				lgdprintf(D_TERM, "term_exitfunc: closing fd[%d]=%d:\n",
				   i, term.fd[i]);
				lgdprintf(D_TERM, "term_exitfunc: fd[%d]=%d\n", term.fd[i], i);
				if (!(tname = ttyname(term.fd[i]))) {	// skip if not tty
					if (errno == ENOTTY)
						lgdprintf(D_TERM, "term_exitfunc: not a tty\n");
						continue;
				}

				r = tcflush(term.fd[i], TCIOFLUSH);
				lgdprintf(D_TERM, "term_exitfunc: tcflushed r=%d\n", r);
				if ( r < 0 ) break;
				r = tcsetattr(term.fd[i], TCSAFLUSH, &term.origtermios[i]);
				lgdprintf(D_TERM, "term_exitfunc: tcsetattr r=%d\n", r);
				if ( r < 0 ) break;
			} while (0);
			lgdprintf(D_TERM, "term_exitfunc: Using i=%d\n", i);
			if ( r < 0 ) {
				lgdprintf(D_TERM, "term_exitfunc: fd[%d]=%d\n", term.fd[i], i);
	...
	lgdprintf(D_TERM, "term_exitfunc: end\n");
}
...
term_add (int fd)
{
	...
	do { /* dummy */
		...
		term.fd[i] = fd;
		lgdprintf(D_TERM, "term_add: i=%d, fd=%d\n", i, fd);
	} while (0);
...
term_replace (int oldfd, int newfd)
{
	...
	lgdprintf(D_TERM, "term_replace: old=%d, new=%d\n", oldfd, newfd);
	...
	do { /* dummy */
		...
		term.fd[i] = newfd;
		lgdprintf(D_TERM, "term_replace: oldfd=%d, newfd=%d, i=%d\n", 
		    oldfd, newfd, i);
...
term_apply (int fd)
{
	...
	do { /* dummy */
		...
		lgdprintf(D_TERM, "term_apply: fd=%d\n", fd);

		i = term_find(fd);
		if ( i < 0 ) {
			lgdprintf(D_TERM, "term_apply failed\n"); 
			rval = -1;
			break;
		}
		 ...
		 int local, int hup_close)
{
	...
	do { /* dummy */
		...
		lgdprintf(D_TERM, "term_set: fd=%d\n", fd);
		i = term_find(fd);
		lgdprintf(D_TERM, "term_set: i=%d, terr=%d\n", i, term_errno );
		if ( i < 0 ) {
			ni = term_add(fd);
			lgdprintf(D_TERM, "term_set: term_add failed %d, terr=%d\n", 
			    ni, term_errno);
			if ( ni < 0 ) {
				rval = -1;
				break;
			...
			ni = i;
			lgdprintf(D_TERM, "term_set: term_add worked ni= %d\n", ni);
		}
		...
		do { /* dummy */
			...
			if (raw) {
				r = term_set_raw(fd);
				lgdprintf(D_TERM, "term_set: term_set_raw=%d, terr=%d\n", 
					r, term_errno);
				if ( r < 0 ) { rval = -1; break; }
			}
			
			r = term_set_baudrate(fd, baud);
			lgdprintf(D_TERM, "term_set: term_set_baudrate=%d, terr=%d\n", 
				r, term_errno);
			if ( r < 0 ) { rval = -1; break; }
			
			r = term_set_parity(fd, parity);
			lgdprintf(D_TERM, "term_set: term_set_parity=%d, terr=%d\n", 
				r, term_errno);
			if ( r < 0 ) { rval = -1; break; }
			
			r = term_set_databits(fd, bits);
			lgdprintf(D_TERM, "term_set: term_set_databits=%d, terr=%d\n", 
				r, term_errno);
			if ( r < 0 ) { rval = -1; break; }
			
			r = term_set_flowcntrl(fd, fc);
			lgdprintf(D_TERM, "term_set: term_set_flowcntrl=%d, terr=%d\n", 
				r, term_errno);
			if ( r < 0 ) { rval = -1; break; }
			
			r = term_set_local(fd, local);
			lgdprintf(D_TERM, "term_set: term_set_local=%d, terr=%d\n", 
				r, term_errno);
			if ( r < 0 ) { rval = -1; break; }
			
			r = term_set_hupcl(fd, hup_close);
			lgdprintf(D_TERM, "term_set: term_set_hupcl=%d, terr=%d\n", 
				r, term_errno);
			if ( r < 0 ) { rval = -1; break; }
	...
	lgdprintf(D_TERM, "term_set: returing %d\n", rval);
	return rval;
}
